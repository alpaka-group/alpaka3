...
TEST_CASE("vec generic", "[vector]")
{
    ...
    // Workaround for CUDA to avoid Catch2 no assert warning. STATIC_REQUIRED uses a function, which is not __host__
    // __device__ annotated. Therefore, it throws compiler warnings if it is used in a kernel.
    STATIC_REQUIRE(true);
}
