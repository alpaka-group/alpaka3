...
The **onAcc** namespace contains the actual algorithms that are executed within a :ref:`kernel` on the ``accelerator``, such as a CPU or GPU.

The ``accelerator`` does not represent a specific processor in the system. It represents a category of processors. A specific processor is a :ref:`Device`.

For example, if we have a system with two Intel Xeon CPUs and four AMD Instinct GPUs.

- The system as two different ``accelerators``: The Intel Xeon CPU and the AMD Instinct GPU.
- It has 6 :ref:`Devices <Device>`: CPU 0 and 1 and GPU 0 until 3

The host selects a :ref:`Device`, but the kernel is written for an abstract ``accelerator``, since it can be executed on different :ref:`Devices <Device>`.

Properties:
    - **onAcc** can only be entered from the **onHost** by enqueuing a compute :ref:`kernel`.
