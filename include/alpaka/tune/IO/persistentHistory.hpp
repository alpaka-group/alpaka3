...
namespace alpaka::tune::IO
{
    ...
    public:
        ...
        std::size_t read(
            KernelTuningModel<ModelArgs...> const& model,
            alpaka::tune::IO::RuntimeHistory<T_Config>& history,
            KernelTuningMetadata const& metadata,
            ...
            if(soft.contains("configs") && soft["configs"].is_array())
            {
                for(auto const& jcfg : soft["configs"])
                {
                    ...
                    for(auto const& m : jcfg["measurements"])
                    {
                        ...
                        ::alpaka::tune::config::updateMetrics<false, T_MetricInterface>(entry, state, val);
                    }
                }
            }
            return loaded;
        ...
        std::size_t write(
            KernelTuningModel<ModelArgs...> const& model,
            RuntimeHistory<T_Config> const& history,
