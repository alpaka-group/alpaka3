...
namespace alpaka::tune::concepts
{
    ...
    template<auto Tag>
    struct isShallowTunableDummy<detail::ShallowTunableDummy<Tag>> : std::true_type
    {
    };
    ...
    template<typename T>
    concept TuneableLike = runtimeTuneable<T> || isShallowTunableDummy_v<T> || std::is_same_v<T, detail::NoTune>;
    template<typename T>
