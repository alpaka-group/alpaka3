...
/**
 * @namespace alpaka::onHost::tune::Vars
 * @brief Holds runtime-accessible limits/break criteria for various TuningSessions and setters honoring
 * env precedence.
 * currently supported environmentVariables:
 *  TunerMaxTotalConfigs = maximum number of distinct paramater configurations to be checked (valid + non-valid)
 *  TunerMaxValidConfigs = maximum number of distinct parameter configurations to be executed (only valid)
 *  TunerRunsPerConfig = maximum amount of times a single paramater configuration is executed AND !measured!
 *              (NOTE: this does not include warm-up runs -- the first run for any config is usually discarded)
 *
 */
namespace alpaka::onHost::tune::Vars
{
