...
#include <alpaka/onHost/tune/adjust/api.hpp>
#include <alpaka/onHost/tune/core/TuningContextManager.hpp>
...
#include <alpaka/onHost/tune/store/PersistentHistory.hpp>
...
#include <alpaka/onHost/tune/utils/transformKernelBundle.hpp>
...
namespace alpaka::onHost::tune
{
    ...
    namespace internal::core
    {
        ...
        public:
            ...
            peripherals::ConfigQueue<tune::config::ConfigRecord<T_Config>> env_config_queue;
...

            {
                ...
                env_environmentState.maxConfigsTotal
                    = std::min(Vars::getMaxTotalConfigs(), env_tuningModel.getMaxPossibleRuns());
                env_environmentState.maxValidEvaluations
                    = std::min(env_environmentState.maxConfigsTotal, Vars::getMaxValidConfigs());
                // ---
                ...
                tune::config::ConfigRecord<T_Config>& firstEntry = getHistory().getOrCreate(initConfigTmp);
            ...
            TuningSession<T_Args...> const& session)
        {
            ...
            auto CTuneableBundle
                = compileTimeHelpers::getCTunables<typename std::remove_cvref_t<T_KernelBundle>::KernelFn>();
            ...
            using T_Config = tune::config::Config<uint32_t, T_completeTuningModel::numDims>;
            // define type for tuning Context
            using tuningEnvironmentType = TuningContext<
                T_Config,
