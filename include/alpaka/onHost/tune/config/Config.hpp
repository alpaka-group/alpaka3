...
template<alpaka::onHost::tune::concepts::Integral T, auto N>
struct std::hash<alpaka::onHost::tune::config::Config<T, N>>
{
    std::size_t operator()(alpaka::onHost::tune::config::Config<T, N> const& c) const noexcept
    {
        std::size_t seed = 0;
        std::hash<T> hasher;
