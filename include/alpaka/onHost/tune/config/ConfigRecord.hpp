...
#include <alpaka/onHost/tune/config/MetricContainer.hpp>
...
namespace alpaka::onHost::tune
{
    ...
    namespace config
    {
        ...
        struct ConfigRecord
        {
             ...
             *
             * pushMetric is only called during the active tuning process (we did not reach the global break criteria
             * yet)
             *
            ...
            void pushMetric(double_t const val)
            {
                ...
                case internal::config::ConfigState::Retired:
                    // append nothing and do nothing (avoids draining system memory, when very heavy workloads are
                    // performed in dense tuning space) this could be changed in the future (to implement some form of
                    // dynamic best config selection) since pushMetric only occurs during the active tuning process
            ...
            internal::config::ConfigState state = internal::config::ConfigState::Uninitialized;
            // Data
